...
def create_table_data(filtered_data, locales):
    """
...
def by_group_view(stories):
    ...
    if grouped_data:
        ...
        table_data = create_table_data(filtered_data, locales)
...
def page_language_grouping_app(dev_mode=False):
    ...
    group_tab, page_tab = st.tabs(["By Group", "Each page level"])
    with group_tab:
        by_group_view(stories)
    with page_tab:
        by_page_view(stories)
